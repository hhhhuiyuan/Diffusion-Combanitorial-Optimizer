...
class TSPModel(COMetaModel):
  ...
  def forward(self, x, adj, t, edge_index, sparse_inf=None):
    return self.model(x, t, adj, edge_index, sparse_inf=sparse_inf)
  ...
  def categorical_training_step(self, batch, batch_idx):
    ...
    if self.args.sparse_noise:
      xt = xt * (1.0 + 0.05 * (2*torch.rand_like(xt)-1))
    else:
  ...
  def categorical_denoise_step(self, points, xt, t, device, edge_index=None, target_t=None, sparse_inf=False):
    with torch.no_grad():
      ...
      x0_pred = self.forward(
          ...
          sparse_inf = sparse_inf,
      )

      if not self.sparse or self.args.sparse_noise:
    ...
    for _ in range(self.args.sequential_sampling):
      ...
      for i in range(steps):
        ...
        if self.diffusion_type == 'gaussian':
          ...
          if self.args.sparse_noise:
            xt = self.categorical_denoise_step(
                points, xt, t1, device, edge_index, target_t=t2, sparse_inf=True)
  ...
  #   if not self.args.decoding_strategy == 'greedy':
