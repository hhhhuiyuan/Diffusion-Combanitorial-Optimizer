import numpy as np
from . import lkh_wrapper as lkh_wrapper
...
def get_lower_adj_matrix(adj_matrix):
  '''
  extract the lower left of a numpy djaceny matrix
  '''
  dim = int(adj_matrix.shape[0])
  lower_left_matrix = []
  for i in range(0, dim):
      matrix_d = []
      for j in range(0, i + 1):
        assert adj_matrix[i, j] == adj_matrix[j, i]
        matrix_d.append(adj_matrix[i, j])
      lower_left_matrix.append(matrix_d)
  return lower_left_matrix
