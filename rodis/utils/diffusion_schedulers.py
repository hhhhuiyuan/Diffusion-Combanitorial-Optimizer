...
class CategoricalDiffusion(object):
  """Categorical Gaussian Diffusion process"""

  def __init__(self, T, schedule, sparse_noise_prob=None):
    # Diffusion steps
    self.T = T
    # Noise schedule
    if schedule == 'linear':
    ...
    if sparse_noise_prob:
      ones = np.ones((2, 2)).reshape((1, 2, 2))
      ones = 2 * ones * np.array([1 - sparse_noise_prob, sparse_noise_prob])
    else:
      ones = np.ones((2, 2)).reshape((1, 2, 2))
  ...
  def sample(self, x0_onehot, t):
    ...
    if len(x0_onehot.shape) == 3:
      # diffuse on node feature vector
      xt = torch.matmul(x0_onehot, Q_bar)
    else:
      # diffuse on adj matrix
