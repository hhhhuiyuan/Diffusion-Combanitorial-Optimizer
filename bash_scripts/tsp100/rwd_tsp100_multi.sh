...
#submit by $sbatch bash_scripts/rwd_tsp100_multi.sh
...
python -u difusco/train.py \
  ...
  --wandb_logger_name "tsp100_rwd_multi" \
  --diffusion_type "categorical" \
  --learning_rate 0.0002 \
  --weight_decay 0.0001 \
  --lr_scheduler "cosine-decay" \
  --storage_path "../outputs/tsp100_rwd/$(date +%Y-%m-%d)/$(date +%H-%M-%S)" \
  --training_split "/data/shared/huiyuan/tsp100/tsp100_rwd_train.txt" \
